    ...
    *,
    ...
    try:
        ...
        row = connection.execute(
            "SELECT id FROM publications WHERE dblp_key = ?", (dblp_key,)
        ).fetchone()
        if row is None:
        ...
        if not providers:
            _log_fetch_attempt(
                ...
                "unsupported",
                "unsupported_source",
                "publication does not expose a supported fulltext lookup identifier",
            )
            connection.commit()
            return {
                "dblp_key": dblp_key,
        ...
        for provider in providers:
            ...
            if not candidates:
                _log_fetch_attempt(
                    ...
                    "not_found",
                    "not_found",
                    "provider did not return a legal pdf candidate",
                )
                continue
            for candidate in candidates:
                try:
                    stored = _download_and_store_fulltext(
                        connection,
                        publication_id,
                        dblp_key,
                        candidate.provider,
...
def _build_lookup(
    ...
    extra_fields = [
        dict(row)
        for row in connection.execute(
            "SELECT field_name, field_value FROM publication_fields WHERE publication_id = ? ORDER BY field_name, position",
            (publication_id,),
        ).fetchall()
    ]
    doi = None
...
def _download_and_store_fulltext(
    ...
    if len(payload) > MAX_FULLTEXT_PDF_BYTES:
        raise ValueError("pdf exceeds configured size limit")
    if payload.lstrip().startswith(b"<!DOCTYPE html") or payload.lstrip().startswith(
        b"<html"
    ):
    ...
    connection.execute(
        ...
        (
            ...
            artifacts.full_text,
            artifacts.full_text_norm,
            artifacts.image_status,
            json.dumps(artifacts.page_image_paths),
            fetched_at,
        ),
    )
...
def _get_cached_fulltext(
    connection: sqlite3.Connection, publication_id: int
) -> dict[str, object] | None:
    """Load one cached full-text payload and sanitize its file paths for clients."""
    row = connection.execute(
        "SELECT provider, source_url, pdf_url, local_pdf_path, sha256, size_bytes, page_count, full_text, image_status, page_image_paths_json, fetched_at FROM publication_fulltexts WHERE publication_id = ?",
        (publication_id,),
    ).fetchone()
    if row is None:
        return None
    return {
        ...
        "page_image_paths": [
            relative_to_data_dir(item)
            for item in json.loads(row["page_image_paths_json"] or "[]")
        ],
