...
from dblp_mcp.fulltext.extract import _validate_not_presentation, PdfArtifacts
from dblp_mcp.fulltext.service import fetch_publication_fulltext, fetch_publication_fulltexts
...
def test_fetch_publication_fulltext_stores_openalex_pdf(database_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr('dblp_mcp.fulltext.providers.openalex.urlopen', fake_openalex_urlopen)
    monkeypatch.setattr('dblp_mcp.fulltext.service.urlopen', lambda request, timeout=60: FakeResponse(_pdf_bytes(), request.full_url))
    monkeypatch.setattr('dblp_mcp.fulltext.service.extract_pdf_artifacts', lambda pdf_path: PdfArtifacts('OpenAlex full text', 'openalex full text', 1, 'unsupported', []))
...
def test_fetch_publication_fulltext_uses_cache(database_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr('dblp_mcp.fulltext.providers.openalex.urlopen', lambda request, timeout=30: FakeResponse(json.dumps({'locations': [{'pdf_url': 'https://dl.acm.org/doi/pdf/10.1000/openalex-test?download=true', 'landing_page_url': 'https://doi.org/10.1000/openalex-test'}]}).encode('utf-8'), request.full_url, 'application/json'))
    monkeypatch.setattr('dblp_mcp.fulltext.service.urlopen', lambda request, timeout=60: FakeResponse(_pdf_bytes(), request.full_url))
    monkeypatch.setattr('dblp_mcp.fulltext.service.extract_pdf_artifacts', lambda pdf_path: PdfArtifacts('Cached full text', 'cached full text', 1, 'unsupported', []))

    first = fetch_publication_fulltext(database_path, 'journals/test/OpenAlex2024')
    assert first['status'] == 'fetched'

    monkeypatch.setattr('dblp_mcp.fulltext.providers.openalex.urlopen', lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError('provider should not be used on cache hit')))
...
def test_fetch_publication_fulltext_logs_unsupported(database_path: Path) -> None:
    ...
    try:
        row = connection.execute('SELECT provider, status, error_code FROM fulltext_fetch_logs ORDER BY id DESC LIMIT 1').fetchone()
    finally:
        connection.close()
...
def test_fetch_publication_fulltexts_returns_mixed_results(database_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr('dblp_mcp.fulltext.providers.openalex.urlopen', lambda request, timeout=30: FakeResponse(json.dumps({'locations': [{'pdf_url': 'https://dl.acm.org/doi/pdf/10.1000/openalex-test?download=true', 'landing_page_url': 'https://doi.org/10.1000/openalex-test'}]}).encode('utf-8'), request.full_url, 'application/json'))
    monkeypatch.setattr('dblp_mcp.fulltext.service.urlopen', lambda request, timeout=60: FakeResponse(_pdf_bytes(), request.full_url))
    monkeypatch.setattr('dblp_mcp.fulltext.service.extract_pdf_artifacts', lambda pdf_path: PdfArtifacts('Bulk full text', 'bulk full text', 1, 'unsupported', []))

    result = fetch_publication_fulltexts(database_path, ['journals/test/OpenAlex2024', 'journals/corr/abs-2401-12345', 'conf/test/Unsupported2024', 'missing/key'])
    assert result['summary'] == {'requested': 4, 'fetched': 2, 'cached': 0, 'unsupported': 1, 'not_found': 0, 'missing': 1, 'error': 0}


def test_get_database_status_reports_fulltext_counts(database_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr('dblp_mcp.fulltext.providers.openalex.urlopen', lambda request, timeout=30: FakeResponse(json.dumps({'locations': [{'pdf_url': 'https://dl.acm.org/doi/pdf/10.1000/openalex-test?download=true', 'landing_page_url': 'https://doi.org/10.1000/openalex-test'}]}).encode('utf-8'), request.full_url, 'application/json'))
    monkeypatch.setattr('dblp_mcp.fulltext.service.urlopen', lambda request, timeout=60: FakeResponse(_pdf_bytes(), request.full_url))
    monkeypatch.setattr('dblp_mcp.fulltext.service.extract_pdf_artifacts', lambda pdf_path: PdfArtifacts('Status full text', 'status full text', 1, 'unsupported', []))
...
def test_ieee_provider_builds_stamp_pdf_candidate(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda request, timeout=30: FakeResponse(b'', 'https://ieeexplore.ieee.org/document/8651517/', 'text/html'),
    )

    candidates = IeeePdfProvider().fetch_candidates(FulltextLookup(doi='10.1109/LRA.2019.2901656'))
...
def test_acm_provider_builds_direct_pdf_candidate() -> None:
    ...
    candidates = AcmPdfProvider().fetch_candidates(FulltextLookup(doi='10.1145/3583740.3626623'))

    assert len(candidates) == 1
    assert candidates[0].provider == 'acm_pdf'
    assert candidates[0].pdf_url == 'https://dl.acm.org/doi/pdf/10.1145/3583740.3626623?download=true'
    assert candidates[0].request_headers is not None
    assert candidates[0].request_headers['Referer'] == 'https://dl.acm.org/doi/10.1145/3583740.3626623'


def test_fetch_publication_fulltext_returns_relative_pdf_path(database_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr('dblp_mcp.fulltext.providers.openalex.urlopen', lambda request, timeout=60: FakeResponse(json.dumps({'locations': [{'pdf_url': 'https://dl.acm.org/doi/pdf/10.1000/openalex-test?download=true', 'landing_page_url': 'https://doi.org/10.1000/openalex-test'}]}).encode('utf-8'), request.full_url, 'application/json'))
    monkeypatch.setattr('dblp_mcp.fulltext.service.urlopen', lambda request, timeout=60: FakeResponse(_pdf_bytes(), request.full_url))
    monkeypatch.setattr('dblp_mcp.fulltext.service.extract_pdf_artifacts', lambda pdf_path: PdfArtifacts('Relative path text', 'relative path text', 1, 'unsupported', []))
...
def test_fetch_publication_fulltext_respects_pdf_size_limit(database_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr('dblp_mcp.fulltext.providers.openalex.urlopen', lambda request, timeout=60: FakeResponse(json.dumps({'locations': [{'pdf_url': 'https://dl.acm.org/doi/pdf/10.1000/openalex-test?download=true', 'landing_page_url': 'https://doi.org/10.1000/openalex-test'}]}).encode('utf-8'), request.full_url, 'application/json'))
    monkeypatch.setattr('dblp_mcp.fulltext.service.urlopen', lambda request, timeout=60: FakeResponse(b'%PDF-1.4' + b'x' * 32, request.full_url))
