...
def test_get_recent_fetch_failures_returns_abstract_and_fulltext_entries(tmp_path: Path) -> None:
    ...
    try:
        ...
        publication_id = connection.execute("SELECT id FROM publications WHERE dblp_key = ?", ('journals/test/Diag2024',)).fetchone()[0]
        log_abstract_attempt(connection, publication_id=publication_id, dblp_key='journals/test/Diag2024', provider='openalex', attempted_url='https://api.openalex.org/x', status='not_found', error_code='not_found', error_message='no abstract')
        log_fulltext_attempt(connection, publication_id, 'journals/test/Diag2024', 'openalex_pdf', 'https://example.org/x.pdf', 'error', 'HTTPError', 'bad response')
        connection.commit()
    finally:
        connection.close()
...
def test_get_recent_fetch_failures_validates_arguments(tmp_path: Path) -> None:
    ...
    with connect(database_path) as connection:
        connection.execute("CREATE TABLE IF NOT EXISTS publications (id INTEGER PRIMARY KEY, dblp_key TEXT)")
...
def test_get_recent_fetch_failures_filters_by_category(tmp_path: Path) -> None:
    ...
    abstract_only = get_recent_fetch_failures(database_path, category='abstract', limit=10)
    fulltext_only = get_recent_fetch_failures(database_path, category='fulltext', limit=10)
